    ...
    func setState(_ state: Bool, animated: Bool) -> TimeInterval {
        ...
        if animated  {
            if _isOn {
                ...
                let groupAction = SCNAction.group([switchAction, .playSoundEffect(Switch.openSound)])
                ...
                let groupAction = SCNAction.group([switchAction, .playSoundEffect(Switch.closeSound)])
                let action = SCNAction.sequence([waitAction, groupAction])
