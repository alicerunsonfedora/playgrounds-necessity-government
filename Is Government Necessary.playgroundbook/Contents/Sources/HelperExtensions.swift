...
let π = SCNFloat(Float.pi) // (Option + p) for convenience when using with SceneKit
...
extension OptionSet {
    func isIntersecting(with set: Self) -> Bool {
        return !isDisjoint(with: set)
    }
}
