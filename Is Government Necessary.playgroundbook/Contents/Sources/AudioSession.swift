...
class AudioSession {
     ...
     */
    private var voiceOverWasRunningAtLastStatusChange = UIAccessibilityIsVoiceOverRunning()
    ...
    private init() {
        ...
        if #available(iOS 11, *) {
            voiceOverStatusDidChangeNotificationName = .UIAccessibilityVoiceOverStatusDidChange
        }
        else {
            voiceOverStatusDidChangeNotificationName = Notification.Name(rawValue: UIAccessibilityVoiceOverStatusChanged)
        }
        ...
        let voiceOverStatusChanged = nc.addObserver(forName: voiceOverStatusDidChangeNotificationName, object: nil, queue: .main) { [weak self] notification in
            ...
            let voiceOverIsRunning = UIAccessibilityIsVoiceOverRunning()
        ...
        let secondaryAudio = nc.addObserver(forName: .AVAudioSessionSilenceSecondaryAudioHint, object: nil, queue: .main) { [weak self] notification in
            ...
            else if isPlayingSecondaryAudio && !UIAccessibilityIsVoiceOverRunning() && !isAnnouncingVoiceOverOff {
