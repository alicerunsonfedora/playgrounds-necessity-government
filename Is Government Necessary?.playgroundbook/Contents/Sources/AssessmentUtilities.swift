...
extension GridWorld.SuccessCriteria: MessageConstructible {
    var message: PlaygroundValue {
        ...
        case .ignoreGoals:
            // Show no values if you `.ignoreGoals`.
        ...
        case let .custom(criteria, _):
            guard case let .array(additionalValues) = criteria.message else {
                fatalError("Invalid success criteria message found.")
            }
...
/// Wraps an assessment function for ease of creating `.custom(_)` assessment.
...
extension GridWorld {
    ...
    public func charactersInOrder() -> Bool {
        ...
        if existingGems(at: characterPlacements).count >= 1 {
            return false
        }
        return true
    }
