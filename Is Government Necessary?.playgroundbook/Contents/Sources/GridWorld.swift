...
public class GridWorld {
    ...
    public enum SuccessCriteria {
        ...
        /// When no gems or switches need to be tracked on a puzzle.
        case ignoreGoals
        
        /// A function used to evaluate custom assessment.
        indirect case custom(SuccessCriteria, AssessmentHandler)
    ...
    public var successCriteria: SuccessCriteria = .allGoals
    
    public var columnCount = 0
    
    public var rowCount = 0
    
    public var isCharacterPickerEnabled = true
    ...
    // Set several times during key building phases of the world to ensure it's always up to date.
    //
    ...
    /**
     ...
     - localizationKey: GridWorld.allPossibleCoordinates
     */
    ...
    func shouldShowPicker(from node: SCNNode) -> Bool {
        // Exit early if the character picker is disabled via `isCharacterPickerEnabled`.
        guard isCharacterPickerEnabled else { return false }
        ...
        // Filter out experts.
        let onlyActors = grid.actors.filter { gridActor in
            let actorType = type(of: gridActor)
            return actorType == Actor.self
        }
...
extension GridWorld : AccessibilityComponentProtocol {
    func speakText(for command: Action) -> String {
        ...
        case .remove(let indices):
            for item in items(from: indices) where type(of: item) != RandomNode.self {
...
extension GridWorld {
    ...
    /// - localizationKey: GridWorld.existingItems(at:)
    ...
    /// - localizationKey: GridWorld.existingItems(ofType:)
    public func existingItems<Child: Item>(ofType itemType: Child.Type) -> [Child] {
        return existingItems(ofType: itemType, at: allPossibleCoordinates)
    }
    
    /// Returns the first occurrence of the specified type at the given coordinates.
    /// - localizationKey: GridWorld.existingItem(ofType:at{Coordinate}:)
    public func existingItem<Child: Item>(ofType itemType: Child.Type, at coordinate: Coordinate) -> Child? {
        return existingItems(ofType: itemType, at: [coordinate]).first
    }
    
    /// Returns all nodes with the provided identifier that exist at the given coordinates.
    /// - localizationKey: GridWorld.existingItems(ofType:at{[Coordinate]}:)
    public func existingItems<Child: Item>(ofType itemType: Child.Type, at coordinates: [Coordinate]) -> [Child] {
    ...
    /// - localizationKey: GridWorld.excludingNodes(ofTypes:at:)
    func excludingNodes<Child: Item>(ofTypes types: [Child.Type], at coordinates: [Coordinate]) -> [Item] {
        return coordinates.flatMap { coordinate -> [Item] in
            ...
            return candidateChildren.filter { child in
                ...
                return !types.contains { $0 == childType }
            }
        }
    }
    
    func excludingNodes<Node: Item>(ofType itemType: Node.Type, at coordinate: Coordinate) -> [Item] {
        return excludingNodes(ofTypes: [itemType], at: [coordinate])
    }
    ...
    /**
     ...
     - localizationKey: GridWorld.place(_:facing:at:)
     */
    @discardableResult
    ...
    /**
     ...
     - localizationKey: GridWorld.place(nodeOfType:facing:at:)
     */
    @discardableResult
    ...
    /**
     ...
     - localizationKey: GridWorld.place(_:between:and:)
     */
    public func place(_ portal: Portal, between start: Coordinate, and end: Coordinate) {
        let linkedPortal = Portal(color: portal.color, shape: portal.shape)
        portal.linkedPortal = linkedPortal
...
extension GridWorld {
    ...
    /**
     ...
     - localizationKey: GridWorld.column(_:)
     */
    ...
    /**
     ...
     - localizationKey: GridWorld.row(_:)
     */
    ...
    /**
     ...
     - localizationKey: GridWorld.coordinates(inColumns:)
     */
    ...
    /**
     ...
     - localizationKey: GridWorld.coordinates(inRows:)
     */
    ...
    ````
     ...
     - localizationKey: GridWorld.coordinates(inColumns:intersectingRows)
     */
    ...
    ````
     ...
     - localizationKey: GridWorld.height(at:)
     */
...
extension GridWorld {
    ...
    /**
     ...
     - parameters:
        - coordinates: Takes an array of type `Coordinate`, specifying a list of locations in the puzzle world.
     - localizationKey: GridWorld.existingGems(at:)
     */
    ...
    /**
     ...
     - parameters:
        - coordinates: Takes an array of type `Coordinate`, specifying a list of locations in the puzzle world.
     - localizationKey: GridWorld.existingSwitches(at:)
     */
    ...
    /**
     ...
     - parameters:
        - coordinates: Takes an array of type `Coordinate`, specifying a list of locations in the puzzle world.
     - localizationKey: GridWorld.existingCharacters(at:)
     */
    ...
    /**
     ...
     - parameters:
        - coordinates: Takes an array of type `Coordinate`, specifying a list of locations in the puzzle world.
     - localizationKey: GridWorld.existingExperts(at:)
     */
    ...
    /**
     ...
     - parameters:
        - coordinates: Takes an array of type `Coordinate`, specifying a list of locations in the puzzle world.
     - localizationKey: GridWorld.existingBlocks(at:)
     */
    ...
    /**
     ...
     - localizationKey: GridWorld.numberOfBlocks(at:)
     */
    ...
    /**
     ...
     - localizationKey: GridWorld.topBlock(at:)
     */
    ...
    /**
     ...
     - localizationKey: GridWorld.existingWater(at:)
     */
    ...
    Method that places multiple blocks into the puzzle world, using an array of coordinates.
     - localizationKey: GridWorld.placeBlocks(at:)
    */
    @discardableResult
    ...
    Method that places multiple water tiles into the puzzle world, using an array of coordinates.
     - localizationKey: GridWorld.placeWater(at:)
    */
    @discardableResult
    ...
    Method that places multiple gems into the puzzle world, using an array of coordinates.
     - localizationKey: GridWorld.placeGems(at:)
    */
    @discardableResult
    ...
    Method that places an item into the puzzle world.
     ...
     - localizationKey: GridWorld.place(_:atColumn:row:)
     */
    ...
    /**
     ...
     - parameters:
        ...
        - column: Takes an Int specifying a column of the puzzle world.
        - row: Takes an Int specifying a row of the puzzle world.
     - localizationKey: GridWorld.plac(_:facing:atColumn:row:)
     */
    ...
    /**
     ...
     - localizationKey: GridWorld.plac(_:atStartColumn:startRow:atEndColumn:endRow:)
     */
    ...
    /**
     ...
     - parameters:
        - column: Takes an Int specifying a column of the puzzle world.
        - row: Takes an Int specifying a row of the puzzle world.
     - localizationKey: GridWorld.removeItems(atColumn:row:)
     */
    ...
    /**
     ...
     - parameters:
         - column: Takes an Int specifying a column of the puzzle world.
         - row: Takes an Int specifying a row of the puzzle world.
     - localizationKey: GridWorld.removeBlock(atColumn:row:)
     */
    public func removeBlock(atColumn column: Int, row: Int) {
        removeBlock(at: Coordinate(column: column, row: row))
    }
    ...
    /**
     ...
     - localizationKey: GridWorld.removeBlock(at:)
     */
    public func removeBlock(at coordinate: Coordinate) {
       removeBlocks(at: coordinate, onlyRemoveTopBlock: true)
    }
    ...
    /**
     ...
     - parameters:
         - at: Takes a `Coordinate` specifying a location in the puzzle world.
     - localizationKey: GridWorld.removeAllBlocks(at:)
     */
    public func removeAllBlocks(at coordinate: Coordinate) {
        removeBlocks(at: coordinate)
    }
    ...
    /**
     ...
     - parameters:
         ...
         - row: Takes an Int specifying a row of the puzzle world.
     - localizationKey: GridWorld.removeAllBlocks(atColumn:row:)
     */
    public func removeAllBlocks(atColumn column: Int, row: Int) {
        removeAllBlocks(at: Coordinate(column: column, row: row))
    }
    
    private func removeBlocks(at coordinate: Coordinate, onlyRemoveTopBlock: Bool = false) {
        for node in grid.nodes(at: coordinate).reversed() {
            guard let _ = node as? Block else { continue }
            node.removeFromWorld()
            if onlyRemoveTopBlock { break }
        }
    }
