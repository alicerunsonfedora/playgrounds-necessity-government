...
struct NodeFactory {
    ...
    static func make(with identifier: WorldNodeIdentifier, in world: GridWorld, from stateInfo: [PlaygroundValue]) -> Item? {
        ...
        case .portal:
            guard stateInfo.count >= 3,
                case let .boolean(on) = stateInfo[0],
                case let .data(colorData) = stateInfo[1],
                let shape = PortalShape(playgroundValue: stateInfo[2]),
                let color = NSKeyedUnarchiver.unarchiveObject(with: colorData) as? _Color
                else { return nil }

            let portal = Portal(color: Color(color), shape: shape)
            portal.isActive = on
            node = portal
